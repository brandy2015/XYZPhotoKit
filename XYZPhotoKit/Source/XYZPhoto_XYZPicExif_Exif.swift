...
public struct Exif {
    ...
    public init(ciimage: CIImage) {
        ...
        self.dictionaryX = dictionary!
    }
    var dictionaryX:Dictionary<String, Any> = [:]
    public init(dictionary: Dictionary<String, Any>?) {
        ...
        //fileprivate let keys = Array.ini
        let pathx = Bundle.main.url(forResource: "ExifKeys", withExtension: "plist")!
//        let DataX = try! Data(contentsOf: pathx)
        let arrayX = NSArray(contentsOf: pathx)!
        let keys = arrayX as! Array<String>
//        let keys  DataX as! NSArray
        //
